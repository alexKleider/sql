...
class Test_keys_from_query(unittest.TestCase):
    ...
    def test_keys_from_complex_query(self):
        query = """     /* taken from Sql/applicants.sql */
        SELECT St.key, P.first, P.last, 
        ...
        ORDER BY St.key;
        """
        keys = [ "St_key", "P_first", "P_last", 
            "P_phone", "P_address", "P_town", "P_state",
            "P_postal_code", "P_email",
            "sponsor1ID", "sponsor2ID",
            "app_rcvd", "fee_rcvd", "meeting1", "meeting2", "meeting3",
            "approved", "dues_paid", "St_text",  ]
        keys_from_query = routines.keys_from_query(query,
                                       replace_periods=True)
        self.assertEqual(keys, keys_from_query)
